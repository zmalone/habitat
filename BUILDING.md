...
work has been run through a nightly version of rustfmt. An easy way to
...
to your `PATH`. Actually running it would look something like `rustup run
nightly cargo fmt --all` from the root of the project. The actual toolchain
name can vary, depending on which version of nightly rust you have installed.
