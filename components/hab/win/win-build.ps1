...
if (-not (Test-AppVeyor)) {
    ...
    if (!(Test-Path "/hab/cache/keys/core-*.sig.key")) {
        Write-Host "System is missing core origin signing key"
        Write-Host "You will be prompted to paste in the secret key now."
        Write-Host ""
        $originKey = Read-Host "Paste your key now"
        $originKey | & '..\..\..\target\Release\hab.exe' origin key import
    }

    # Create signed .hart file
    & '..\..\..\target\Release\hab.exe' pkg sign --origin core "$pkgArtifact.Zip" "$pkgArtifact.hart"
    Remove-Item -Path "$pkgArtifact.Zip"
