...
use self::{
    ...
    service::{
        ConfigRendering, DesiredState, HealthCheck, Service, ServiceProxy, ServiceSpec, Topology,
    },
...
use habitat_butterfly::{
    member::Member,
    server::{timing::Timing, Server as ButterflyServer, ServerProxy, Suitability},
    trace::Trace,
};
use habitat_common::{
    command,
...
use habitat_launcher_client::{LauncherCli, LAUNCHER_LOCK_CLEAN_ENV, LAUNCHER_PID_ENV};
        ...
        ops.into_iter().filter_map(|op| {
            match op {
                ServiceOperation::Stop(spec) | ServiceOperation::Restart{to_stop: spec, ..} => {
                    ...
                    // restart future.
                    let f = self.remove_service_from_state(&spec).map(|service| self.stop(service));
                    if f.is_none() {
