...
fn env_from(package: &PackageInstall) -> Result<Env> {
    let mut env = package.runtime_environment()?;
    let mut paths: Vec<PathBuf> = match env.get(ENV_PATH_KEY) {
