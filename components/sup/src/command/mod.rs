...
use super::{PROGRAM_NAME, VERSION};
use config::{gcache, gconfig, Command, Config, GossipListenAddr, UpdateStrategy, Topology};
use http_gateway;
use output;
...
fn config_from_args(subcommand: &str, sub_args: &ArgMatches) -> Result<()> {
    ...
    config.set_path(sub_args.value_of("path")
        .unwrap_or(fs::svc_path(PROGRAM_NAME.as_str())
            .join("data")
            .to_string_lossy()
            .as_ref())
        .to_string());
...
pub fn run_sup(test_args: Option<Vec<&str>>) -> Result<()> {
    let arg_url = || {
        Arg::with_name("url")
            .short("u")
            .long("url")
    ...
    let args = App::new(PROGRAM_NAME.as_str())
        .version(VERSION)
    ...
    let matches = match test_args {
        Some(fake_args) => try!(args.get_matches_from_safe(fake_args)),
        None => try!(args.get_matches_safe()),
    };
    ...
    try!(config_from_args(subcommand_name, &subcommand_matches));

    match gconfig().command() {
